...
namespace RoboPioneers::Prometheus
{
	...
	/// 更新方法
	void ColorPerceptionService::OnUpdate(const Sparrow::Frame &frame)
	{
		if (Settings.TargetColor == TargetColorEnum::Red)
		{
			/// 筛选红色区域
			Output.MaskPicture = FilterRedArea(*Input.HSVPicture);
		}
		else if (Settings.TargetColor == TargetColorEnum::Blue)
		{
			/// 筛选蓝色区域
			Output.MaskPicture = FilterBlueArea(*Input.HSVPicture);
		}
