...
namespace RoboPioneers::Prometheus
{
	...
	void BattleIntelligenceService::OnUpdate(const Sparrow::Frame &frame)
	{
		DebugPictureIntelligence = frame.Picture.clone();

//		if (CurrentStatus == StatusType::Search)
//		{
//			if (UpdateSearchStatus(Input.PossibleArmors))
//			{
//				CurrentStatus = StatusType::Track;
//			}
//		}

		Output.InterestedAreas = cv::Rect(0,0,0,0);
		...
		if (found)
		{
			...
			{
				interested_area.x = interested_area.x + Input.PositionOffset->x;
				interested_area.y = interested_area.y + Input.PositionOffset->y;
				...
				Output.X = (std::get<0>(best_pair).Center.x + std::get<1>(best_pair).Center.x ) / 2 + Input.PositionOffset->x;
				Output.Y = (std::get<0>(best_pair).Center.y + std::get<1>(best_pair).Center.y ) / 2 + Input.PositionOffset->y;
		...
		if (Tracked)
		{
			Output.InterestedAreas = TrackingInterestedArea;
		}
		...
		{
			best_one.center = best_one.center + Modules::MathUtility::ChangePointType<float>(*Input.PositionOffset);
